...
const NATIVE_CURRENCY = { name: 'CELO', symbol: 'CELO', decimals: 18 }

/**
 * Attempts to add the required Celo network to the connected wallet
...
export async function tryAddRequiredChain(chainId?: number): Promise<boolean> {
  ...
  try {
    await provider.request({
      ...
      console.log('✅ Added Celo network via wallet_addEthereumChain')
    return true
    } catch (error: any) {
    ...
    if (account.connector) {
      ...
      if (connector.addChain) {
        try {
          ...
          console.log('✅ Added Celo network via connector.addChain')
          return true
      ...
      if (connectorProvider?.request) {
        try {
          ...
          console.log('✅ Added Celo network via connector provider')
          return true
        } catch (providerError) {
